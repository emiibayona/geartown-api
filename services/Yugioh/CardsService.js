    ...
    try {
        ...
        const getAllAndParse = async () => {
            ...
            for (const element of cards?.rows) {
                if (!element.local_url || !element.local_url.includes("cloudinary")) {
                    const res = await filesService.getImage({ game: query.game, id: element.name, folder: 'yugioh' }, { url: element?.card_images[0]?.image_url, skipFirstFetch: true })
